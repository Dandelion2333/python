...
def loadDataSet():
    if log_data == 1:
        ...
        dataSet = []
        dataSet = get_source_data.get_2_co_occurrence()
...
if __name__ == '__main__':
    ...
    print_log.print_source(myDat)

    L, suppData = apriori( myDat, 0 )                       # 选择频繁项集
    # print(u"频繁项集L：", L)
    # print(u"所有候选项集的支持度信息：", suppData)
    print_log.print_support(suppData)

    rules = generateRules( L, suppData, minConf = 0 )
    # print('rules:\n', rules)
    print_log.print_confidence(rules)

    # 把置信度生成表格数据
    head.set_file_type()
    create_confidence.confidence_main()